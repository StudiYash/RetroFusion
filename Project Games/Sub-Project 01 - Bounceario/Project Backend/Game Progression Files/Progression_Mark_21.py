...
# Reserve channels:
# bg_channel: channel 0 used for background music via Sound objects (to avoid lag on looping)
...
# Load sound effects (located in the 'audios' folder)
...
# Distance thresholds for powerup spawns
...
def spawn_mushroom_powerup():
    ...
    # Do not play sound here; sound is handled upon collision.
    if ball is None:
        return
...
# Note: The collection (collision) of a mushroom will trigger its sound in check_obstacle_collisions.
...
class Ball:
    ...
    def update(self, keys, ground, platforms):
        ...
        # When jump is initiated (from on ground to not on ground), trigger jump sound
    ...
    for mush in mushrooms[:]:
        if mask_collision(ball, mush):
            ...
            try:
                ...
                pass
            # Now play mushroom collection sound and switch bg music
            mush_collect_channel.play(mushpowercollect_sound)
            switch_background_music_sound(mush_bg_sound, fade_out=500, fade_in=200)
    # Handle gun collision
    for gun in guns[:]:
...
def draw_play_screen():
    ...
    if ball and ball.invincible:
        ...
        if remaining_inv < 0:
            ...
            switch_background_music_sound(main_bg_sound, fade_out=500, fade_in=200)
        else:
    ...
    if game_state == "start":
        ...
        if ball:
            ...
            if ball and check_obstacle_collisions(ball):
                bg_channel.stop()  # Stop background music immediately
