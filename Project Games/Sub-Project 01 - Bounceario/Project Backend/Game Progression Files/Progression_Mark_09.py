...
BALL_COLOR = (255, 0, 0)
OBSTACLE_COLOR = (0, 200, 0)  # Green obstacles (static ones)
...
def update_ground_obstacles(camera_x):
    ...
    while last_x < camera_x + screen_width + 500:
        ...
        if random.random() < 0.05:  # 5% chance for ground static obstacle
            sample_dx = 5
...
# --- Moving Obstacles (New Non-Static Obstacles) ---
# These obstacles appear on the ground and move from right to left following the ground's slope.
class MovingObstacle(Obstacle):
    def __init__(self, x, y, width, height, angle, speed):
        super().__init__(x, y, width, height, on_platform=False, angle=angle)
        self.speed = speed  # constant speed (pixels per frame)

    def update(self):
        # Move left at constant speed
        self.x -= self.speed
        # Update y to remain on the ground
        self.y = ground.get_ground_height(self.x) - self.height
        # Recompute the angle based on the ground slope
        sample_dx = 5
        gh1 = ground.get_ground_height(self.x)
        gh2 = ground.get_ground_height(self.x + sample_dx)
        self.angle = math.atan2(gh2 - gh1, sample_dx)

# Global list for moving obstacles
moving_obstacles = []
...
def update_moving_obstacles(camera_x):
    ...
    # Remove moving obstacles that have gone off-screen
    moving_obstacles = [m for m in moving_obstacles if m.x + m.width > camera_x - 100]
    # Ensure spawn tracker is at least off-screen to the right
    if last_moving_obstacle_x < camera_x + screen_width:
        last_moving_obstacle_x = camera_x + screen_width + 100
    # Spawn new groups of moving obstacles on the ground
    while last_moving_obstacle_x < camera_x + screen_width + 500:
        # Choose a group size with lower frequency than static obstacles.
        ...
        for i in range(group_size):
            ...
            speed = random.uniform(2, 4) * 0.5  # now between ~1 and 2 pixels per frame
            moving_obstacles.append(MovingObstacle(new_x, new_y, obs_width, obs_height, angle, speed))
        last_moving_group_size = group_size
        # Advance the spawn tracker further for the next group
        last_moving_obstacle_x += random.randint(500, 800)

# --- Collision Detection ---
def check_collision(ball, obstacle):
    # Collision detection based on the obstacle's axis-aligned bounding box.
...
def check_obstacle_collisions(ball):
    # Check collisions with static obstacles and moving obstacles
    all_obstacles = ground_obstacles + platform_obstacles + moving_obstacles
...
# Initial coin and moving obstacle spawn tracking
last_ground_coin_x = 0
last_moving_obstacle_x = 0
last_moving_group_size = 0
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_state == "start" and event.key == pygame.K_SPACE:
                ...
                platform_coins = []
                last_ground_coin_x = 0
                # Set moving obstacles spawn tracker off-screen to the right
                last_moving_obstacle_x = camera_x + screen_width + 100
                camera_x = 0
                score = 0
                # Reset coin_spawned flag for platforms
                for p in floating_platforms:
