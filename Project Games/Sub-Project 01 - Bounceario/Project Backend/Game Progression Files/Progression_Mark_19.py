...
# --- Global variables for distance-based powerup spawning ---
MUSHROOM_DISTANCE_THRESHOLD = 10000    # (Modified: spawn a mushroom every 10,000 pixels)
GUN_DISTANCE_THRESHOLD = 8000            # (Modified: spawn a gun every 8,000 pixels)
last_mushroom_spawn_x = 0
last_gun_spawn_x = 0

# Global list for bullets (old global timers not used anymore)
bullets = []
...
class FloatingPlatform:
    def __init__(self, x, y, width, height=10):
        ...
        self.last_pickup_time = 0  # (Time-based value no longer used for powerups)
        self.has_powerup = False   # True if a powerup is already on this platform
        self.obstacle_destroyed = False  # Once an obstacle is destroyed, no new ones appear
...
def update_floating_platforms(camera_x):
    ...
    # If no platforms exist, start generating from x = 0 for early level access.
...
class Obstacle:
    def __init__(self, x, y, angle, on_platform=False):
        ...
        # For death animation and shot handling:
        self.dying = False
...
class Gun:
    def __init__(self, x, y, angle, on_platform=False):
        self.x = x
        self.y = y
        # Adjust gun image size to 50px width and 35px height.
        self.width = 50   
        self.height = 35  
        self.on_platform = on_platform
        self.angle = angle
...
def update_platform_obstacles(camera_x):
    ...
    # Remove off-screen platform obstacles.
    platform_obstacles = [o for o in platform_obstacles if o.x + OBSTACLE_SIZE > camera_x - 100]
    
    # For each floating platform, spawn a standard obstacle (if eligible) with a small chance.
    ...
    # After the standard spawn loop, check for distance-based powerup spawn.
    if ball:
        spawn_mushroom_powerup()
        spawn_gun_powerup()

# --- Functions for Distance-Based Powerup Spawning ---
