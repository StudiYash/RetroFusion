...
last_mushroom_collect_time = -80000  # 80 seconds
last_gun_collect_time = -150000      # 150 seconds
...
class FloatingPlatform:
    def __init__(self, x, y, width, height=10):
        ...
        self.has_powerup = False  # New flag: if True, no further obstacles spawn on this platform
...
def update_floating_platforms(camera_x):
    ...
    # Platforms are cleared on new game.
...
class Gun:
    def __init__(self, x, y, angle, on_platform=False):
        self.x = x
        self.y = y
        self.width = 50   # New: 50px width
        self.height = 35  # New: 35px height
        self.on_platform = on_platform
        self.angle = angle

    def draw(self, surface, camera_x):
        sx = self.x - camera_x
        # Scale gun image to (50,35)
        scaled_img = pygame.transform.scale(gun_image, (self.width, self.height))
        rotated_img = pygame.transform.rotate(scaled_img, -math.degrees(self.angle))
        rect = rotated_img.get_rect()
        # Align so that the bottom center aligns with the platform (use self.height for vertical offset)
...
def update_ground_obstacles(camera_x):
    ...
    # Reduce ground obstacle spawn chance by 15% (i.e. use 0.05*0.85 ≈ 0.0425)
...
def update_platform_obstacles(camera_x):
    ...
    for p in floating_platforms:
        # Only spawn on platforms that do NOT already have a powerup!
        if p.width < 150 or p.has_powerup:
            continue
        # (We keep the 2-second delay if a pickup was already spawned on this platform)
        ...
        if not exists and random.random() < 0.03:
            ...
            # Next 10% chance for gun pickup if cooldown permits
                ...
                # (Do not set has_powerup for standard obstacles)
...
def check_obstacle_collisions(ball):
    ...
    for mush in mushrooms[:]:
        if mask_collision(ball, mush):
            current_time = pygame.time.get_ticks()
            if ball.invincible:
                ball.invincible_end_time += 20000  # Add 20 seconds
            else:
            ...
            for p in floating_platforms:
                if p.x <= mush.x <= p.x + p.width and abs(p.y - (mush.y + OBSTACLE_SIZE)) < 10:
                    p.last_pickup_time = current_time
            # (Note: even if collected, the platform remains marked if it had a pickup)
    # Handle gun collisions
    for gun in guns[:]:
        if mask_collision(ball, gun):
            current_time = pygame.time.get_ticks()
            if ball.gun_power:
                ball.gun_power_end_time += 60000  # Add 60 seconds
            else:
                ball.gun_power = True
...
# Early Powerup Spawn on Game Start: Force-spawn one mushroom and one gun on platforms with x < 2000.
def spawn_early_powerups():
    mushroom_spawned = False
    gun_spawned = False
    for p in floating_platforms:
        if p.x < 2000 and not p.has_powerup:
            if not mushroom_spawned:
                # Spawn a mushroom on this platform
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_state in ("start", "gameover") and event.key == pygame.K_SPACE:
                ...
                spawn_early_powerups()  # Force early powerups within first 2000 px.
                last_ground_coin_x = 0
                last_moving_obstacle_x = 0
                camera_x = 0
                score = 0
