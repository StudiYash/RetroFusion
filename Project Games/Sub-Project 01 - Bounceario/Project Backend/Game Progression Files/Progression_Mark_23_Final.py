...
pygame.mixer.init()
pygame.mixer.set_num_channels(10)  # Ensure channels 0..9 are available

# Reserve channels:
bg_channel = pygame.mixer.Channel(0)         # Background music
...
knockdown_channel = pygame.mixer.Channel(8)
life_channel = pygame.mixer.Channel(9)

# Load sound effects (from "audios" folder)
...
knockdown_sound = pygame.mixer.Sound("audios/knockdownspider.mp3")
life_sound = pygame.mixer.Sound("audios/life.mp3")
pause_sound = pygame.mixer.Sound("audios/pause.mp3")  # New pause sound

# Load background music as Sound objects
main_bg_sound = pygame.mixer.Sound("audios/mainbg.mp3")
mush_bg_sound = pygame.mixer.Sound("audios/mushpowerbg.mp3")

# Global variables for background music switching via timer
pending_bg_music = None
pending_fade_in = 0
BG_MUSIC_SWITCH_EVENT = pygame.USEREVENT + 1  # Custom event

def play_background_music_sound(sound, fade_in=0):
    bg_channel.play(sound, loops=-1, fade_ms=fade_in)

def switch_background_music_sound(new_sound, fade_out=2000, fade_in=2000):
    global pending_bg_music, pending_fade_in
    bg_channel.fadeout(fade_out)
    pending_bg_music = new_sound
    pending_fade_in = fade_in
    pygame.time.set_timer(BG_MUSIC_SWITCH_EVENT, fade_out, True)
...
gun_image = pygame.image.load("assets/gun.png").convert_alpha()
...
BALL_RADIUS = 20
MAX_BOUNCE_MULTIPLIER = 5  # ≤2: short jump; ≥3: long jump
...
# Global distance thresholds for powerups
MUSHROOM_DISTANCE_THRESHOLD = 10000
GUN_DISTANCE_THRESHOLD = 8000
last_mushroom_spawn_x = 0
last_gun_spawn_x = 0

# Life system variables
lives = 1                        # Player starts with 1 life
next_life_score_threshold = 2000 # Extra life awarded at current score + 2000, max 3 lives

# Other global variables
bullets = []
gameover_start_time = None
respawn_start_time = None   # For respawn delay (now 2 seconds)
paused = False              # For pause toggle
pause_start_time = 0        # To record when pause begins
# -------------------- End Game Parameters --------------------

# -------------------- Background Music Control --------------------
def start_playing_background():
    play_background_music_sound(main_bg_sound, fade_in=0)
...
class FloatingPlatform:
    def __init__(self, x, y, width, height=10):
        ...
        self.last_pickup_time = 0  # Not used for powerups now
...
def spawn_mushroom_powerup():
    ...
    if ball.x - last_mushroom_spawn_x >= MUSHROOM_DISTANCE_THRESHOLD:
        ...
        if candidate is not None:
            ...
            # Collection sound and bg switch occur upon collision
...
class Ball:
    def __init__(self, x, y):
        ...
        self.invincible = False   # Only granted by mushroom powerup
        self.invincible_end_time = 0
        self.gun_power = False      # Gun power only grants bullet firing
    ...
    def update(self, keys, ground, platforms):
        # Dual control: Use both WASD and Arrow keys.
        ...
        # For jump: use W or Up Arrow.
        if current_on_surface and (keys[pygame.K_w] or keys[pygame.K_UP]):
        ...
        self.key_was_pressed = (keys[pygame.K_w] or keys[pygame.K_UP])
...
ball = None
game_state = "start"  # States: "start", "playing", "respawn", "dying", "gameover", "paused"
...
def check_obstacle_collisions(ball):
    ...
    for obs in all_obs:
        if mask_collision(ball, obs):
            if not obs.dying:
                obs.dying = True
                obs.death_velocity = -12
            if obs.on_platform:
                for p in floating_platforms:
                    if p.x <= obs.x <= p.x+p.width and abs((obs.y+OBSTACLE_SIZE)-p.y) < 10:
                        p.obstacle_destroyed = True
            # If ball is mushroom-invincible, play knockdown sound. Otherwise, fatal.
            if ball.invincible:
                knockdown_channel.play(knockdown_sound)
            else:
    ...
    for mush in mushrooms[:]:
        if mask_collision(ball, mush):
            ...
            switch_background_music_sound(mush_bg_sound, fade_out=2000, fade_in=2000)
    for gun in guns[:]:
...
def draw_play_screen():
    ...
    score_text = score_font.render(f"Score: {score}", True, (0,0,255))
    screen.blit(score_text, (20,20))
    # Draw hearts (lives) at top-center with 5px spacing.
    total_hearts = lives
    if total_hearts > 0:
        heart_width = 25
        heart_spacing = 5
        total_width = total_hearts * heart_width + (total_hearts - 1) * heart_spacing
        start_x = (screen_width - total_width) // 2
        y_pos = 5
        for i in range(total_hearts):
            x_pos = start_x + i*(heart_width+heart_spacing)
            screen.blit(heart_image, (x_pos, y_pos))
    # Only update powerup timers if state is "playing"
    if ball and game_state == "playing":
        if ball.invincible:
            remaining_inv = (ball.invincible_end_time - pygame.time.get_ticks()) / 1000
            if remaining_inv < 0:
                ball.invincible = False
                switch_background_music_sound(main_bg_sound, fade_out=2000, fade_in=2000)
            else:
                inv_text = score_font.render(f"Invincible: {int(remaining_inv)} seconds", True, (255,255,0))
                screen.blit(inv_text, (screen_width-300,20))
        if ball.gun_power:
            ...
            if remaining_gun < 0:
                ...
                screen.blit(gun_text, (screen_width-300,60))
    # If game is paused, display "Pause" in the center.
    if game_state == "paused":
        pause_text = pygame.font.SysFont("Arial", 72).render("Pause", True, (255,255,255))
        pause_rect = pause_text.get_rect(center=(screen_width//2, screen_height//2))
        screen.blit(pause_text, pause_rect)
    pygame.display.flip()
# -------------------- End Screen Drawing Functions --------------------
...
def start_playing_background():
    play_background_music_sound(main_bg_sound, fade_in=0)
# -------------------- End Main Background Music Start --------------------

# -------------------- Life System Variables --------------------
lives = 1
next_life_score_threshold = 2000
# -------------------- End Life System Variables --------------------
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            ground_min_y = int(screen_height * 0.7)
            ground_max_y = screen_height - 50
        elif event.type == BG_MUSIC_SWITCH_EVENT:
            if pending_bg_music is not None:
                bg_channel.play(pending_bg_music, loops=-1, fade_ms=pending_fade_in)
                pending_bg_music = None
            ...
            if event.key == pygame.K_p:
                ...
                if paused:
                    ...
                    bg_channel.pause()
                    pause_sound.play()  # Play pause sound
                    game_state = "paused"
                else:
                    # Calculate pause duration; update powerup timers
                    pause_duration = current_time - pause_start_time
                    if ball:
                        if ball.invincible:
                            ball.invincible_end_time += pause_duration
                        if ball.gun_power:
                            ball.gun_power_end_time += pause_duration
                    bg_channel.unpause()
                    pause_sound.play()  # Play pause sound when unpausing
                    game_state = "playing"
    ...
    if paused:
        # If paused, just redraw the frame with pause overlay; do not update game state.
        draw_play_screen()
        clock.tick(FPS)
        continue

    # Award extra life every 2000 points, based on current score.
    if score >= next_life_score_threshold and lives < 3 and game_state == "playing":
        lives += 1
        life_channel.play(life_sound)
        next_life_score_threshold = score + 2000
    ...
    if game_state == "start":
        ...
        if ball:
            ...
            if ball and check_obstacle_collisions(ball):
                if lives > 1:
                    ...
                    death_channel.play(death_sound)
                    game_state = "dying"
                    death_animation_start_time = current_time
                    ball.velocity_y = -12
        ...
        if current_time - respawn_start_time >= 2000:
            ground_obstacles = [o for o in ground_obstacles if o.x > ball.x+1000]
            platform_obstacles = [o for o in platform_obstacles if o.x > ball.x+1000]
            moving_obstacles = [o for o in moving_obstacles if o.x > ball.x+1000]
            ground_coins = [c for c in ground_coins if c.x > ball.x+1000]
            platform_coins = [c for c in platform_coins if c.x > ball.x+1000]
            # Set ball to spawn from the top so it falls naturally.
            ball.y = -ball.radius
            ball.velocity_y = 0
            start_playing_background()
            game_state = "playing"
        ...
        if elapsed >= 3000:
            game_state = "gameover"
            gameover_start_time = current_time
            bg_channel.stop()
