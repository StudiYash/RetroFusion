...
pygame.mixer.init()
pygame.mixer.set_num_channels(9)  # Ensure channels 0-8 are available

# Reserve channels:
# Channel 0 for background music (using Sound objects)
...
# Load sound effects from the "audios" folder
...
# Load background music as Sound objects (preloaded to avoid lag on looping)
main_bg_sound = pygame.mixer.Sound("audios/mainbg.mp3")
mush_bg_sound = pygame.mixer.Sound("audios/mushpowerbg.mp3")

# Global variable for background music switching via a timer
pending_bg_music = None
pending_fade_in = 0
BG_MUSIC_SWITCH_EVENT = pygame.USEREVENT + 1  # Custom event for switching bg music
...
BALL_RADIUS = 20
MAX_BOUNCE_MULTIPLIER = 5  # Bounce multiplier: ≤2 => short jump; ≥3 => long jump
...
def spawn_mushroom_powerup():
    ...
    if ball.x - last_mushroom_spawn_x >= MUSHROOM_DISTANCE_THRESHOLD:
        ...
        if candidate is not None:
            ...
            # Note: The mushroom collection sound and bg switch occur upon collision.
...
class Ball:
    def __init__(self, x, y):
        ...
        self.invincible = False  # Granted only by the mushroom powerup.
        self.invincible_end_time = 0
        self.gun_power = False   # Gun powerup affects only bullets.
...
ball = None
game_state = "start"  # States: "start", "playing", "dying", "gameover"
...
def check_obstacle_collisions(ball):
    ...
    for obs in all_obs:
        if mask_collision(ball, obs):
            ...
            # If ball is invincible (mushroom), play knockdown sound.
            # Otherwise, a fatal collision occurs.
            if ball.invincible:
    ...
    for gun in guns[:]:
        if mask_collision(ball, gun):
            current_time = pygame.time.get_ticks()
            # Gun powerup now grants ONLY bullet-firing ability, not invincibility.
            if ball.gun_power:
...
def draw_play_screen():
    ...
    screen.blit(score_text, (20, 20))
    y_offset = 20
    # Only if the game state is "playing" do we update bg music on mushroom expiration.
    if ball and ball.invincible and game_state == "playing":
        ...
        if remaining_inv < 0:
            ...
            y_offset += 40
    if ball and ball.gun_power and game_state == "playing":
    ...
    if game_state == "start":
        ...
        if ball:
            ...
            if ball and check_obstacle_collisions(ball):
                bg_channel.stop()  # Immediately stop background music
