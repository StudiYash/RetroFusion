...
OBSTACLE_SIZE = 50           # Base size for obstacles (used for ground obstacles)
OBSTACLE_HOVER_OFFSET = 10
SAFE_PADDING = 20
MAX_COIN_ATTEMPTS = 10

# --- Global cooldowns for pickups (in ms) ---
last_mushroom_collect_time = -80000  # initial value
last_gun_collect_time = -150000      
...
class FloatingPlatform:
    def __init__(self, x, y, width, height=10):
        ...
        self.last_pickup_time = 0  # Time when a powerup was spawned on this platform
        self.has_powerup = False   # If True, no further obstacles spawn on this platform
        self.obstacle_destroyed = False  # New flag: once an obstacle is destroyed, no new ones appear
        self.powerup_type = None   # New attribute to store which powerup ("mushroom" or "gun") is active
...
def update_floating_platforms(camera_x):
    ...
    # Remove platforms that are too far left
    floating_platforms = [p for p in floating_platforms if p.x + p.width > camera_x - 100]
    # Modification: If no platforms exist, start generating from x = 0 for early level access.
...
class Obstacle:
    def __init__(self, x, y, angle, on_platform=False):
        ...
        # Added for death animation and shot handling:
        self.dying = False
...
class Gun:
    def __init__(self, x, y, angle, on_platform=False):
        self.x = x
        self.y = y
        # Modification: Adjust gun image size to 50px width and 35px height.
        self.width = 50   
        self.height = 35  
        self.on_platform = on_platform
        self.angle = angle
...
def update_ground_obstacles(camera_x):
    ...
    # Reduced ground obstacle spawn chance by 15%.
...
def update_platform_obstacles(camera_x):
    ...
    for p in floating_platforms:
        # Reset platform powerup flag if cooldown has elapsed even if not picked up.
        if p.has_powerup:
            if p.powerup_type == "mushroom" and (current_time - p.last_pickup_time >= 40000):
                p.has_powerup = False
                p.powerup_type = None
            elif p.powerup_type == "gun" and (current_time - p.last_pickup_time >= 90000):
                p.has_powerup = False
                p.powerup_type = None
        # Skip platforms that are too small, already have a powerup, or have had an obstacle destroyed.
        if p.width < 150 or p.has_powerup or p.obstacle_destroyed:
            continue
        # Keep a delay if a pickup was already spawned on this platform.
        ...
        if not exists and random.random() < 0.03:
            ...
            # Spawn a mushroom if spawn_choice < 0.1 and global cooldown of 40 seconds has passed.
            if spawn_choice < 0.1 and (current_time - last_mushroom_collect_time >= 40000):
                ...
                last_mushroom_collect_time = current_time
            # Spawn a gun if spawn_choice is between 0.1 and 0.2 and global cooldown of 90 seconds has passed.
            elif spawn_choice >= 0.1 and spawn_choice < 0.2 and (current_time - last_gun_collect_time >= 90000):
                ...
                p.has_powerup = True
                p.powerup_type = "gun"
                last_gun_collect_time = current_time
            else:
                # Spawn a standard obstacle only if no powerup is there.
                ...
                # Standard obstacles do not change the has_powerup flag.
...
def check_obstacle_collisions(ball):
    ...
    for obs in all_obs:
        if mask_collision(ball, obs):
            if not obs.dying:
                obs.dying = True
                obs.death_velocity = -12  # initial upward impulse for death animation
            # If the obstacle is on a platform, mark that platform as having its obstacle destroyed.
            if obs.on_platform:
                for p in floating_platforms:
                    if p.x <= obs.x <= p.x + p.width and abs((obs.y + OBSTACLE_SIZE) - p.y) < 10:
                        p.obstacle_destroyed = True
            # Only register collision (for ball death) if the ball is vulnerable and the obstacle was not shot.
            if not ball.invincible and not obs.shot:
                collision_detected = True
    # Handle mushroom collisions (invincibility stacking)
    for mush in mushrooms[:]:
        if mask_collision(ball, mush):
            current_time = pygame.time.get_ticks()
            if ball.invincible:
                ball.invincible_end_time += 20000  # Add 20 seconds if already invincible
            else:
    ...
    for gun in guns[:]:
        if mask_collision(ball, gun):
            current_time = pygame.time.get_ticks()
            if ball.gun_power:
                ball.gun_power_end_time += 60000  # Add 60 seconds if already powered
            else:
                ball.gun_power = True
...
# Modification: Early Powerup Spawn on Game Start within the first 1000 pixels.
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_state in ("start", "gameover") and event.key == pygame.K_SPACE:
                ...
                spawn_early_powerups()  # Force early powerups within first 1000 px.
                last_ground_coin_x = 0
                last_moving_obstacle_x = 0
                camera_x = 0
                score = 0
    ...
    if game_state == "start":
        ...
        if ball:
            ...
            # Always process obstacle collisions so that obstacles show death animation.
        ...
        # Update dying animations for obstacles.
        ...
        # Remove obstacles that are dying and off-screen.
        ...
        for bullet in bullets[:]:
            ...
            if bullet.x - camera_x > screen_width:
                ...
                continue
            hit = False
            # Check bullet collisions against non-pickup obstacles.
            for obs in ground_obstacles + moving_obstacles + [o for o in platform_obstacles if not isinstance(o, (Mushroom, Gun))]:
                if mask_collision(bullet, obs):
                    if not obs.dying:
                        obs.dying = True
                        obs.death_velocity = -10  # impulse for bullet impact
                        obs.shot = True
                        if obs.on_platform:
                            for p in floating_platforms:
                                if p.x <= obs.x <= p.x+p.width and abs((obs.y+OBSTACLE_SIZE)-p.y) < 10:
