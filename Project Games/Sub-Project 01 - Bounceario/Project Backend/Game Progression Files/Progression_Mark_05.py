...
class Ball:
    def __init__(self, x, y):
        ...
        # For detecting landing events
        self.was_on_surface = True
        self.key_was_pressed = False  # for edge detection on the Up key

    def get_surface(self, ground, platforms):
        # Start with the continuous ground height.
        candidate = ground.get_ground_height(self.x)
        # For each platform spanning the ball's x, check if the ball's bottom swept through its top.
        for p in platforms:
            if p.x <= self.x <= p.x + p.width:
                # If in the previous frame the bottom was above the platform and now it's at or below (within tolerance)
    ...
    def update(self, keys, ground, platforms):
        ...
        # Save previous vertical position.
        self.prev_y = self.y
        ...
        # --- Jump (Bounce) Logic ---
        # If the ball is on a surface (ground or platform):
        if current_on_surface:
            # Check for a new Up key press OR a landing event.
            # If either (a) the Up key was not pressed in the previous frame (edge detection)
            # or (b) the ball just transitioned from airborne to on-surface, trigger a jump.
            if (not self.key_was_pressed or not self.was_on_surface) and keys[pygame.K_UP]:
                ...
                # Mark the ball as now airborne.
                current_on_surface = False
        else:
            # If not on surface, do nothing special.
            pass

        # Reset bounce multiplier if Up key is not held.
