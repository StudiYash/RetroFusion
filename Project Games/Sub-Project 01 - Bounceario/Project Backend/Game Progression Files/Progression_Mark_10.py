...
class Obstacle:
    ...
    def draw(self, surface, camera_x):
        ...
        if not self.on_platform:
            ...
            local_v1 = (-self.width / 2, 0)
            local_v2 = (self.width / 2, 0)
            local_v3 = (0, -self.height)
            ...
            vertices = [
                (sx, self.y + self.height),
                (sx + self.width, self.y + self.height),
                (sx + self.width / 2, self.y)
            ]
            pygame.draw.polygon(surface, OBSTACLE_COLOR, vertices)
...
# --- Moving Obstacles (Non-Static Obstacles) ---
...
def update_coins(camera_x):
    ...
    while last_ground_coin_x < camera_x + screen_width + 500:
        ...
        # Check against fixed ground obstacles
        collision = False
        for obs in ground_obstacles:
            if check_collision(temp_coin, obs):
                collision = True
                break
        if not collision:
