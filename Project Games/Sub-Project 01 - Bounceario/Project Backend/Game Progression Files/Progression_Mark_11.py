...
# Now load images from assets folder
spider_image = pygame.image.load("assets/spider.png").convert_alpha()
coin_image = pygame.image.load("assets/coin.png").convert_alpha()
ball_image_orig = pygame.image.load("assets/ball.png").convert_alpha()  # keep original for rotation

# Colors and font (for background and ground)
BACKGROUND_COLOR = (20, 20, 20)
GROUND_COLOR = (255, 255, 255)
# Ball color fallback is not used since we use an image.
BALL_COLOR = (255, 0, 0)
...
# --- Obstacles (Static) using Spider Image with Pixel-Perfect Masks ---
class Obstacle:
    def __init__(self, x, y, width, height, on_platform=False, angle=0):
        self.x = x
        self.y = y  # top-left of bounding box
        self.width = width
        self.height = height
    ...
    def draw(self, surface, camera_x):
        sx = self.x - camera_x
        # Scale the spider image to obstacle size
        scaled_img = pygame.transform.scale(spider_image, (self.width, self.height))
        # Rotate image by -angle in degrees (pygame rotates counterclockwise)
        rotated_img = pygame.transform.rotate(scaled_img, -math.degrees(self.angle))
        rect = rotated_img.get_rect()
        # Position the image so that its bottom center is at (sx + width/2, y + height)
        rect.center = (sx + self.width/2, self.y + self.height)
...
class Ball:
    ...
    def get_mask(self):
        # Scale and rotate the ball image
    ...
    def update(self, keys, ground, platforms):
        ...
        if keys[pygame.K_RIGHT]:
            self.rotation_angle += 5  # rotate clockwise
        elif keys[pygame.K_LEFT]:
            self.rotation_angle -= 5  # rotate anticlockwise
